...
public class JDBCProjectEx3 extends JFrame implements ActionListener{
	...
	JTextField txtNo, txtName, txtEmail, txtPhone; // default String
	...
	PreparedStatement pstmtSearchScroll, pstmtSearch;
	...
	public void dbConnect() {
		try {
			...
			pstmtTotalScroll = conn.prepareStatement(sqlSelect, 
													ResultSet.TYPE_SCROLL_SENSITIVE, //커서 이동 자유롭게 하고, 업데이트 내용을 반영 
...
//			System.out.println(strNo + strName + strEmail + strPhone);
	...
	public void total() { // 전체보기(select all) 버튼 이벤트 처리 함수
		try {
			...
			if(model == null) model = new MyModel(); // 객체 생성, 메모리 할당, 생성자함수 자동호출
	...
	public void del() {
		...
		JOptionPane.showMessageDialog(null,"삭제 됐습니다.");
		total();
		init();
	}
	public void search() {
